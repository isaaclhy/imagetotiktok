...
import { wrapTextToLines } from '@/app/lib/canvas-utils';
        ...
        {(isFirstCanvas || isEndingCard) ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center p-4" style={{ pointerEvents: 'none', contain: 'layout style paint' }}>
            <div
              ...
              style={{ width: '75%', aspectRatio, maxHeight: '65%', pointerEvents: 'auto', contain: 'layout style paint', paddingTop: currentCanvasId === '2' ? '1rem' : '35%', paddingLeft: currentCanvasId === '2' ? '1rem' : '0.5rem', paddingRight: currentCanvasId === '2' ? '1rem' : '0.5rem', paddingBottom: currentCanvasId === '2' ? '1.25rem' : '1rem' }}
            >
              {currentCanvasId === '2' ? (
                <>
                  <p className="font-bold text-center" style={{ color: currentCanvas.textColor || '#876e9f', fontSize: `${previewFontSize}px`, lineHeight: '1.4', position: 'absolute', top: '1rem', left: '50%', transform: 'translateX(-50%)', width: 'calc(100% - 2rem)', textAlign: 'center', textDecoration: 'underline', textDecorationThickness: '2px', textUnderlineOffset: '0.25rem' }}>Instructions</p>
                    ...
                    <div className="flex flex-col gap-2 w-full" style={{ marginTop: '35%' }}>
                      {card2Texts.filter((t) => t.text.trim()).map((textItem, idx) => (
                        <div key={idx} className="flex items-center gap-2" style={{ width: '100%', maxWidth: '95%' }}>
                          <span className="flex-shrink-0 flex items-center justify-center font-bold rounded-full" style={{ backgroundColor: textItem.color || '#876e9f', color: '#FFFFFF', width: `${previewFontSize * 0.8}px`, height: `${previewFontSize * 0.8}px`, fontSize: `${previewFontSize * 0.6}px`, minWidth: `${previewFontSize * 0.8}px`, lineHeight: '1' }}>{idx + 1}</span>
                          <p className="font-bold text-left break-words overflow-hidden flex-1" style={{ color: textItem.color, wordWrap: 'break-word', overflowWrap: 'break-word', fontSize: `${previewFontSize}px`, lineHeight: '1.4', minHeight: '1em', contain: 'layout style paint', willChange: 'auto', margin: 0, padding: 0 }}>{textItem.text}</p>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                {canvas.id !== '1' && canvas.id !== 'end' && (
                  ...
                  >
                    <div className="absolute inset-0 flex items-center justify-center p-2 pointer-events-none">
                      <div className="bg-white rounded-lg border-4 border-[#e1c2ff] shadow-lg flex flex-col items-start justify-start pointer-events-none relative overflow-hidden" style={{ width: '75%', aspectRatio, maxHeight: '65%', paddingTop: canvas.id === '2' ? '0.5rem' : '35%', paddingLeft: canvas.id === '2' ? '0.5rem' : '0.25rem', paddingRight: canvas.id === '2' ? '0.5rem' : '0.25rem', paddingBottom: canvas.id === '2' ? '0.75rem' : '0.5rem' }} />
                    </div>
