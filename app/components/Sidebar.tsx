...
export function Sidebar({
  ...
  const tabButtonClass = (id: typeof contentTab) =>
    `w-full py-3 px-4 flex items-center gap-3 rounded-lg transition-colors text-left ${
      contentTab === id
  ...
  const footer = (
    <div className="flex-shrink-0 pt-3 border-t border-zinc-200 dark:border-zinc-700 flex flex-col gap-2 relative">
      ...
      {userInfo ? (
        <div className="flex items-center gap-2 min-w-0">
          <div className="relative min-w-0 flex-1">
            ...
            >
              ...
              <span className="text-sm font-medium text-black dark:text-zinc-50 truncate min-w-0 flex-1">
                {userInfo.display_name || 'User'}
              </span>
              <svg
                className={`w-4 h-4 shrink-0 text-black dark:text-zinc-50 transition-transform ${showUserDropdown ? 'rotate-180' : ''}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex-1 h-10 px-4 rounded-lg bg-black hover:bg-zinc-800 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-white font-semibold text-sm transition-colors flex items-center justify-center gap-2"
      ...
      {showSettingsMenu && <SettingsMenuPanel onClose={() => setShowSettingsMenu(false)} />}
    </div>
  );
  ...
  return (
    <>
      {/* Mobile top bar */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-30 flex items-center gap-2 px-3 h-14 bg-white/95 dark:bg-zinc-900/95 backdrop-blur border-b border-zinc-200 dark:border-zinc-800">
        <button
          type="button"
          onClick={() => setMobileNavOpen(true)}
          className="p-2 -ml-1 rounded-lg text-zinc-700 dark:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800"
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-black dark:text-zinc-50 leading-tight truncate">Bleamies</p>
          <p className="text-[11px] text-zinc-500 dark:text-zinc-400 capitalize truncate">
            {contentTab} · {STUDIO_APPS.find((a) => a.id === selectedAppId)?.label ?? selectedAppId}
          </p>
        </div>
        <button
          type="button"
      ...
      {mobileNavOpen ? (
        <div className="lg:hidden fixed inset-0 z-40">
          ...
          <aside className="absolute left-0 top-0 h-full w-[min(18rem,85vw)] flex flex-col p-3 bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-800 shadow-xl">
            <div className="flex items-center justify-between px-2 py-2 mb-1">
              <h1 className="text-xl font-bold text-black dark:text-zinc-50">Bleamies</h1>
              <button
                type="button"
      ...
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex fixed left-0 top-0 h-screen w-56 shrink-0 flex-col p-3 bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-800 z-10">
        <h1 className="text-xl font-bold text-black dark:text-zinc-50 px-4 py-3 flex-shrink-0">Bleamies</h1>
        {navLinks}
        <div className="flex-1 min-h-4" />
        {footer}
      </aside>

      {/* Mobile settings (top-bar gear) — footer panel only exists inside drawer/desktop sidebar */}
      {showSettingsMenu && !mobileNavOpen ? (
